...
function printResults(results: BalanceResult[]) {
  ...
  for (const r of results) {
    if (r.status === "ok") {
      console.log(
        ` ${pad(r.chain, 17)}| ${pad(`${r.balance} ${r.symbol}`, 22)}| ok`,
      );
    } else {
      console.log(
        ` ${pad(r.chain, 17)}| ${pad("—", 22)}| ${r.error ?? "error"}`,
      );
    }
  }
}
...
function printSummary(
  ...
  console.log("━".repeat(56));
  console.log(
    ` Chains queried:    ${results.length} (${successCount} ok, ${results.length - successCount} errors)`,
  );
  console.log(` Total RPC calls:   ${totalRequests}`);
  console.log(
    ` Session cost:      ${spentPathUSD.toFixed(6)} PathUSD ($${spentPathUSD.toFixed(5)})`,
  );
...
async function waitForFunding(
  ...
  while (Date.now() - start < maxWaitMs) {
    ...
    if (balance > 0n) {
      console.log(
        `Faucet: PathUSD balance confirmed (${formatUnits(balance, 6)} PathUSD)`,
      );
      return;
    }
...
async function fundWallet(address: string): Promise<void> {
  try {
    ...
    if (res.ok) {
      console.log(
        "Faucet: Request accepted — waiting for on-chain confirmation...",
      );
    ...
    console.log(
      `Faucet: Request failed — ${err instanceof Error ? err.message : err}`,
    );
  }
}
...
async function main() {
  ...
  const privateKey =
    (process.env.MPPX_PRIVATE_KEY as `0x${string}`) || generatePrivateKey();
  const account = privateKeyToAccount(privateKey);

  console.log();
  console.log(
    `Wallet:  ${account.address}${isGenerated ? " (auto-generated)" : ""}`,
  );
    ...
    for (const chain of CHAINS) {
      try {
        ...
        process.stdout.write(
          `\r  Queried ${results.length}/${CHAINS.length} chains...`,
        );
      } catch (err) {
        totalRequests++;
        results.push({
