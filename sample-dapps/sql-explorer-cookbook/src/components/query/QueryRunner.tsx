...
export default function QueryRunner({ query, defaultOpen }: QueryRunnerProps) {
  const { data, error, isLoading, run } = useQueryExecution();
          ...
          {query.parameters.map((param) => (
            <div key={param.name} className="flex-1 min-w-[200px]">
              ...
              <div className="flex gap-2">
                ...
                {param.sampleQuery && (
                  <button
                    onClick={() => fetchSampleValue(param.name, param.sampleQuery!)}
