...
export default function SocialCard({ result }: SocialCardProps) {
  ...
  const outcomeTokenSymbol = getTokenSymbolFromAddress(
    result.outcomeToken,
    result.chainId || 8453
  );
  const explorerUrl = getExplorerUrl(result.txHash, result.chainId || 8453);
  ...
  return (
    <div className="space-y-6">
      <Card
        ref={cardRef}
        className="bg-gradient-to-br from-blue-600 to-purple-700 text-white overflow-hidden"
      >
            ...
            <div className="text-center mb-6">
              <div className="flex items-center justify-center gap-2 text-lg">
                ...
                <span>
                  for{" "}
                  <strong>
                    ${result.totalValue.toFixed(2)} worth of{" "}
                    {outcomeTokenSymbol}
                  </strong>
                </span>
              </div>
