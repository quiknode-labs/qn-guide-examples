 ...
 *
 * Usage:
 *   npx tsx decode_payment.ts <PAYMENT-SIGNATURE-HEADER-VALUE>
 *
 * Or with ts-node:
 *   npx ts-node decode_payment.ts <PAYMENT-SIGNATURE-HEADER-VALUE>
 *
 * Example:
...
if (args.length < 1) {
  console.log(
    "Usage: npx tsx decode_payment.ts <PAYMENT-SIGNATURE-HEADER-VALUE>"
  );
  console.log("\nExample:");
...
try {
  ...
  console.log("=== Decoded PAYMENT-SIGNATURE Header ===\n");
  ...
  if (paymentData.payload?.authorization) {
    ...
    console.log(
      `Signature:     ${
        auth.signature ? auth.signature.slice(0, 20) + "..." : "N/A"
      }`
    );
  }
